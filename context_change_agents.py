...
class MBMultiModelContext():
    ...
    def __init__(self,
        ...
        self.shape_SA2 = self.shape_SA+(2,)
        self.last_times = np.zeros(shape_last_SA, dtype=int)
        self.last_change_time = np.zeros(self.shape_SA2, dtype=int)
        self.shape_SASA = (self.size_environment,
                           self.size_actions,
                           self.size_environment,
                           self.size_actions)
        # self.covariation_matrix = np.ones(self.shape_SASA)
        self.covariation_matrix = np.zeros(self.shape_SASA)
        self.alpha_cov = alpha_cov
        self.covariation_couples = np.zeros(self.shape_SASA, dtype=bool)
    ...
    def learn_the_model(self, old_state, reward, new_state, action):
        ...
        self.old_time = self.last_times[old_state][action][ind_rel]
        self.last_times[old_state][action][ind_rel] = self.counter
    ...
    def check_before(self, old_state, action):
        t_before = self.last_change_time[old_state][action][0]
        array_change_t_after = self.last_change_time[:, :, 1]
        cov_before = array_change_t_after > t_before
        return cov_before

    def check_after(self, old_state, action):
        t_after = self.last_change_time[old_state][action][1]
        array_change_t_before = self.last_change_time[:, :, 0]
        cov_after = array_change_t_before < t_after
        return cov_after

    def check_timing(self, old_state, action):
        cov_before = self.check_before(old_state, action)
        cov_after = self.check_after(old_state, action)
        cov_sum = cov_before & cov_after
        return cov_sum

    def update_cov_matrix(self, old_state, action):
        cov_sum = self.check_timing(old_state, action)
        cov_sum[old_state][action] = False

        # Variation with used-not_used

        covar_state = self.covariation_couples[old_state, action, :, :]
        covar_other_state = self.covariation_couples[:, :, old_state, action]
        both_covariation = np.logical_and(covar_state, covar_other_state)
        to_update = np.logical_not(both_covariation)

        # To update when the change still covaries - detail
        # still_good = np.logical_and(both_covariation,cov_sum)
        # to_update = np.logical_or(to_update, still_good)

        to_change = np.logical_and(to_update, cov_sum)

        self.covariation_matrix[old_state, action,
                                to_update] *= (1-self.alpha_cov)
        self.covariation_matrix[to_update,
                                old_state, action] *= (1-self.alpha_cov)
        self.covariation_matrix[old_state, action, to_change] += self.alpha_cov
        self.covariation_matrix[to_change, old_state, action] += self.alpha_cov

        self.covariation_couples[old_state, action, to_update] = False
        self.covariation_couples[old_state, action, to_change] = True
        self.covariation_couples[to_change, old_state, action] = True
        self.covariation_couples[old_state, action, both_covariation] = False

        # To update when the change still covaries - detail
        # self.covariation_couples[old_state, action, still_good] = True

        # print("cov_sum", cov_sum)
        # print("both_cov",both_covariation)
        # print("to_update",to_update)

        # Variation every two

        # not_cov_sum = np.logical_not(cov_sum)
        # self.covariation_matrix[old_state,action,not_cov_sum]*=(1-self.alpha_cov)
        # self.covariation_matrix[not_cov_sum,old_state,action] *=(1-self.alpha_cov)
        # self.covariation_matrix[old_state, action, cov_sum]+=self.alpha_cov
        # self.covariation_matrix[cov_sum, old_state,action]+=self.alpha_cov

        # Variation with sqrt

        # self.covariation_matrix[old_state,action,:,:]*=np.sqrt((1-self.alpha_cov))
        # self.covariation_matrix[:,:, old_state,action] *=np.sqrt((1-self.alpha_cov))
        # self.covariation_matrix[old_state, action, cov_sum]+=self.alpha_cov
        # self.covariation_matrix[cov_sum, old_state,action]+=self.alpha_cov

        self.covariation_matrix = np.round(self.covariation_matrix, 3)

        # print(np.reshape(self.covariation_matrix,(self.size_environment*self.size_actions,
        #                                           self.size_environment*self.size_actions)))

        # self.covariation_matrix[:,:,old_state,action] += self.covariation_matrix[old_state,action,:,:]
        # self.covariation_matrix[old_state,action,:,:] += self.covariation_matrix[:,:,old_state,action]
        # self.covariation_matrix[old_state, action, old_state, action] = 0

        # print(self.covariation_matrix)
    ...
    def swap_with_best_model(self,
        ...
        self.update_cov_matrix(old_state,
                               action)
    ...
    def reassign_counts(self,
        ...
        all_times_SA = np.sort(self.last_times[old_state][action])[::-1]
        len_change = len(to_reassign)
        time_change_2 = all_times_SA[len_change-1]
        if len_change == self.horizon:
            time_change_1 = self.old_time
        else:
            time_change_1 = all_times_SA[len_change]
        self.last_change_time[old_state][action][0] = time_change_1
        self.last_change_time[old_state][action][1] = time_change_2
        # print(old_state, action, time_change_1, time_change_2)
        # print(self.last_times[old_state][action])
    ...
    def get_couples_to_merge(self, old_state, action):
        pass
...
# ---------------------------------------------------------------------------- #
    ...
    def from_distrib_to_half_jensen(self, old_state, action, model_number):
        trans_model = self.all_nSAS[model_number][old_state][action]
        last_trans = self.get_distrib_cur_model(old_state, action)
        nb_experiences = np.sum(last_trans)
        if nb_experiences < self.delay:
            ...
            norm = self.all_nSA[model_number][old_state][action]
            trans_model = (trans_model+last_trans) / (norm+nb_experiences)
            last_trans = last_trans/nb_experiences
            kl = self.kl_div(last_trans, trans_model)
            return kl

    def from_mod_number_to_kl(self, ind1, ind2, old_state, action):
        count1 = self.all_nSAS[ind1][old_state][action].copy()
        count2 = self.all_nSAS[ind2][old_state][action].copy()
        ...
        kl = self.kl_div(distrib1, distrib2)

        return kl

    def from_mod_number_to_half_jen(self, ind1, ind2, old_state, action):
        count1 = self.all_nSAS[ind1][old_state][action].copy()
        count2 = self.all_nSAS[ind2][old_state][action].copy()
        ...
        half_jen = self.kl_div(distrib1, distrib_sum)
        return half_jen

    def from_mod_number_sym_kl(self, ind1, ind2, old_state, action):
        kl1 = self.from_mod_number_to_kl(ind1, ind2, old_state, action)
        kl2 = self.from_mod_number_to_kl(ind2, ind1, old_state, action)
        sym_kl = (kl1+kl2)/2
        return sym_kl

    def find_kl(self, count, last, size, order):
        all_distrib = self.count_to_all_distrib(count,
                                                last,
                                                size,
                                                order,
        ...
        for distrib in all_distrib:
            q = distrib[0]
            p = distrib[1]
            kl = self.kl_div(p, q, epsilon=1e-5)
            all_kl.append(kl)
        return np.array(all_kl)

    def min_sum_kl(self, old_count, new_count, last):
        size = self.size_environment
        kl_old_count = self.find_kl(old_count, last, size, order=True)
        kl_new_count = self.find_kl(new_count, last, size, order=False)
        sum_kl = kl_old_count + kl_new_count
        min_index_kl = np.argmin(sum_kl)
        return min_index_kl

    def find_close_model(self, model_number, old_state, action):
        existing_models = self.find_existing_models(old_state, action)
        existing_models.remove(model_number)
        all_kl_div = []
        for i in range(len(existing_models)):
            kl = self.from_mod_number_to_kl(existing_models[i],
                                            model_number,
                                            old_state,
                                            action)
            all_kl_div.apend(kl)

        closest_model = existing_models[np.argmin(all_kl_div)]
        return closest_model


class RmaxContext(MBMultiModelContext):
    def __init__(self,
                 environment,
                 gamma,
                 horizon=10,
        ...
        self.beta = beta
        self.mod_burst = np.zeros(self.shape_SA)

    def compute_reward_VI(self, old_state, action):
        cond_passages = self.nSA[old_state][action] < self.horizon
        if not cond_passages:
            cur_mod = self.current_model[old_state][action]
            kl = self.from_distrib_to_kl(old_state,
                                         action,
                                         cur_mod)
            cond_threshold = kl > self.exploration_threshold
        if self.model_changed:
            # self.mod_burst+=self.horizon//2
            similar_times = np.logical_not(
                self.check_before(old_state, action))
            self.mod_burst += similar_times * \
                self.covariation_matrix[old_state][action]*self.horizon
            self.mod_burst[self.mod_burst >
                           self.horizon] = self.horizon
            self.mod_burst[old_state][action] = 0

            self.R_VI[self.mod_burst >= 1] = self.Rmax
            # print(self.mod_burst)

        if cond_passages:
            self.R_VI[old_state][action] = self.Rmax
            # self.R_VI[old_state][action] = self.R[old_state][action]
        elif not cond_passages and self.mod_burst[old_state][action] >= 1:
            self.R_VI[old_state][action] = self.Rmax
            self.mod_burst[old_state][action] -= 1
        elif cond_threshold:
            self.R_VI[old_state][action] = self.Rmax
        else:
            self.R_VI[old_state][action] = self.R[old_state][action]
...
class SoftBetaMultiModel(MBMultiModelContext):
    ...
    def __init__(self,
        ...
        self.beta = np.ones(self.size_environment)
        self.alpha = 0.1

    def update_beta(self, current_state, action):
        current_model = self.current_model[current_state][action]
        kl = self.from_distrib_to_kl(current_state,
                                     action,
                                     current_model)

    def get_pref_action(self, state):
        q_values = self.Q[state]
        best_qs = np.flatnonzero(q_values == q_values.max())
        action = np.random.choice(best_qs)
        return action

    def update_beta(self, state):
        action = self.get_pref_action(state)
        current_model = self.current_model[state][action]
        kl = self.from_distrib_to_kl(state,
                                     action,
                                     current_model)
        update = self.alpha * 1/(1+kl)
        self.beta[state] = (1-self.alpha)*self.beta[state]+update

    def choose_action(self, current_state):
        exp_Q = np.exp(self.Q[current_state] * self.beta)
        probas = exp_Q / np.sum(exp_Q)
        action = np.random.choice(np.arange(self.size_actions),
                                  p=probas)
        return action
...
class ConfidenceBonusMultiModel(MBMultiModelContext):
    ...
    def get_kl(self, current_state, action):
        current_model = self.current_model[current_state][action]
        enough_experience = self.check_model_experience(current_model,
                                                        current_state,
                                                        action)
        if enough_experience:
            kl = self.from_distrib_to_kl(current_state,
    ...
    def compute_reward_VI(self, old_state, action):
        kl = self.get_kl(old_state, action)
        # if kl > 0.4 :
        #     self.R_VI[old_state][action] = 1
        # else :
        #     self.R_VI[old_state][action] = self.R[old_state][action]
        self.R_VI[old_state][action] = self.R[old_state][action]+self.alpha*kl
